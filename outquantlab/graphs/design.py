...
import outquantlab.graphs.ui_constants as design
...
def generate_colormap(n_colors: int) -> LinearSegmentedColormap:
    ...
    if n_colors == 1:
        return LinearSegmentedColormap.from_list(
            name=cmap_name, colors=[design.BASE_COLORS[0], design.BASE_COLORS[0]], N=2
        )
    elif n_colors <= len(design.BASE_COLORS):
        return LinearSegmentedColormap.from_list(
            name=cmap_name, colors=design.BASE_COLORS[:n_colors], N=n_colors
        )
    else:
        return LinearSegmentedColormap.from_list(
            name=cmap_name, colors=design.BASE_COLORS, N=n_colors
        )
...
def setup_figure_layout(
    ...
    fig.update_layout(  # type: ignore
        ...
        title={"text": figtitle, "font": design.FIG_TITLE_FONT},
        autosize=True,
...
def get_marker_config(color: str):
    return dict(color=color, line=dict(color=design.COLOR_ADJUSTMENT, width=1))
