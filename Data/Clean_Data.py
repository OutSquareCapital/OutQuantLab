...
import Data.Transform_Data as TransformData
import Data.Get_Data as GetData
...
@staticmethod
def adjust_prices_for_inversion(data_prices_df: pd.DataFrame, colonnes: list) -> pd.DataFrame:
    """
    ...
    for colonne in colonnes:
        ...
        returns = data_prices_df[colonne].pct_change()
        ...
        inverted_returns_df = inverted_returns.to_frame(name=colonne)
        
        # Calculer la courbe d'équité à partir des rendements inversés
        inverted_price = TransformData.equity_curves_calculs(inverted_returns_df)
        
        # Remplacer la colonne originale dans le DataFrame par les nouveaux prix
        data_prices_df[colonne] = inverted_price.squeeze()
    
    return data_prices_df

@staticmethod
def clean_and_process_prices(file_path: str, inversion_list: list = None) -> None:
    """
    Charge les données brutes, ajuste les prix pour éviter les valeurs négatives, traite les rendements, 
    applique le bootstrap et reconstitue les prix. Inverse certaines colonnes pour les standardiser avec les actions si elles sont présentes dans la liste d'inversion.

    Args:
        file_path (str): Chemin vers le fichier CSV contenant les données de prix.
        inversion_list (list): Liste des actifs à inverser pour la standardisation avec les actions (par exemple ['VIXY']).
    """
    ...
    # Inverser certaines colonnes si des actifs de la liste d'inversion sont trouvés
    if inversion_list:
        # S'assurer que les colonnes à inverser existent dans le DataFrame
        columns_to_invert = [col for col in inversion_list if col in processed_prices_df.columns]
        if columns_to_invert:
            processed_prices_df = adjust_prices_for_inversion(processed_prices_df, columns_to_invert)
