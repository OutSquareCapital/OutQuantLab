...
from Indicators import IndicatorMetadata, IndicatorsMethods
...
def load_entities(
    indicators_to_test: dict[str, bool], params_config: dict[str, dict[str, list[int]]]
) -> dict[str, Indicator]:
    entities_functions: dict[str, IndicatorMetadata] = (
        IndicatorsMethods.get_all_indicators()
    )
    entities: dict[str, Indicator] = {}

    for name, indicator_meta_data in entities_functions.items():
        active: bool = indicators_to_test.get(name, False)
        params: dict[str, list[int]] = IndicatorsMethods.determine_params(
            name=name, params_config=params_config
        )
        func = indicator_meta_data.func

        entities[name] = Indicator(
            name=name, active=active, func=func, params_values=params
        )
...
def filter_valid_pairs(params: dict[str, list[int]]) -> list[tuple[int, ...]]:
...
class IndicatorsCollection:
    def __init__(
        ...
        self.entities: dict[str, Indicator] = load_entities(
    ...
    def indicators_params(self) -> list[Indicator]:
        for indicator in self.all_active_entities:
            indicator.param_combos = filter_valid_pairs(params=indicator.params_values)
    ...
    def all_params_config(self) -> dict[str, dict[str, list[int]]]:
        return {
            name: indicator.params_values for name, indicator in self.entities.items()
        }
