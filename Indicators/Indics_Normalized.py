...
def indic(
    registry: dict[str, IndicatorMetadata] = {},
) -> Callable[[IndicatorFunc], IndicatorFunc]:
...
class IndicatorsMethods:
    ...
    def process_param(self, func: IndicatorFunc, param: tuple[int, ...]) -> ArrayFloat:
    ...
    def process_data(self, pct_returns_array: ArrayFloat) -> None:
        ...
        self.log_returns_array = shift_array(
            returns_array=log_returns_np(prices_array=prices_array)
        )
    ...
    def mean_price_macd_trend(
        ...
        return calculate_indicator_on_trend_signal(
    ...
    def median_rate_of_change_macd_trend(
        ...
        median_roc_macd_signal = self.median_rate_of_change_macd(
            LenST, LenLT, MacdLength
        )
        return calculate_indicator_on_trend_signal(
    ...
    def central_rate_of_change_macd_trend(
        ...
        central_roc_macd_signal = self.central_rate_of_change_macd(
            LenST, LenLT, MacdLength
        )
        return calculate_indicator_on_trend_signal(
    ...
    def mean_rate_of_change_normalised_trend(
        ...
        mean_reversion_signal = self.mean_rate_of_change_normalised(
            SignalLength, PLength
        )
    ...
    def skewness_on_kurtosis(self, LenSmooth: int, LenSkew: int) -> ArrayFloat:
        ...
        if LenSkew <= 64:
            skew_on_kurt_signal = np.where(
                relative_kurt < 0, -skewness_array, skewness_array
            )
        else:
            skew_on_kurt_signal = np.where(
                relative_kurt < 0, skewness_array, -skewness_array
    ...
    def relative_skewness_on_kurtosis(self, LenSmooth: int, LenSkew: int) -> ArrayFloat:
        ...
        relative_kurt = relative_normalization(kurtosis_array, 2500)
        if LenSkew <= 64:
            relative_skew_on_kurt_signal = np.where(
                relative_kurt < 0, -relative_skew, relative_skew
            )
        else:
            relative_skew_on_kurt_signal = np.where(
                relative_kurt < 0, relative_skew, -relative_skew
    ...
    def relative_skewness_on_kurtosis_trend(
        ...
        relative_skew_on_kurt_signal = self.relative_skewness_on_kurtosis(
            LenSmooth, LenSkew
        )
        trend_signal = self.mean_rate_of_change(TrendLenST, TrendLenLT)
        return calculate_indicator_on_trend_signal(
    ...
    def relative_directional_volatility(
        ...
        relative_directional_vol_raw = relative_normalization(
            directional_volatility_raw, LenRelative
        )
    ...
    def relative_directional_volatility_trend(
        ...
        relative_directional_vol_signal = self.relative_directional_volatility(
            LenSmooth, LenRelative, LenVol
        )
        trend_signal = self.mean_rate_of_change(TrendLenST, TrendLenLT)
        return calculate_indicator_on_trend_signal(
            trend_signal, relative_directional_vol_signal
        )

    @indic(_indicators_registry)
    def normalised_directional_volatility_trend(
        self,
        LenSmooth: int,
        ...
        normalised_directional_vol_signal = self.normalised_directional_volatility(
            LenSmooth, LenNormalization, LenVol
        )
        trend_signal = self.mean_rate_of_change(TrendLenST, TrendLenLT)
        return calculate_indicator_on_trend_signal(
            trend_signal, normalised_directional_vol_signal
        )
